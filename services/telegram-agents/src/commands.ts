import { getMaestroReport } from "./maestro.js";
import type { BusinessSummaryReport } from "./types.js";

const HELP = `
Команда Maestro готова. Пишите обычным языком — команды не обязательны.

Например:
«У нас мало клиентов. Задайте мне вопросы и разберитесь, что делать»
«Финансист, почему доход растёт медленнее клиентов?»
«Маркетолог и Instagram-продюсер, подготовьте план продвижения»
«Соберите нужных специалистов и предложите план на 30 дней»

Быстрые отчёты и действия:

/today — показатели за сегодня
/week — последние 7 дней
/month — последние 30 дней
/problems — найти главные проблемы бизнеса
/instagram — контент-план Instagram на 7 дней
/reel <тема> — готовый сценарий Reels и задание для Higgsfield
/content [номер] — последние задания или статус одного задания
/approve <номер> — подтвердить расход кредитов и генерацию
/cancel <номер> — отменить задание
/help — эта подсказка
`.trim();

export async function handleDirectCommand(text: string): Promise<string | null> {
  const command = text.trim().split(/\s+/)[0]?.toLowerCase().replace(/@\w+$/, "");
  if (command === "/start" || command === "/help") return HELP;

  const days = command === "/today" ? 1 : command === "/week" ? 7 : command === "/month" ? 30 : null;
  if (!days) return null;

  const report = await getMaestroReport({ action: "business_summary", days }) as unknown as BusinessSummaryReport;
  return formatBusinessSummary(report);
}

export function normalizeUserRequest(text: string): string {
  const command = text.trim().split(/\s+/)[0]?.toLowerCase().replace(/@\w+$/, "");
  if (command === "/problems") {
    return "Найди три главные проблемы бизнеса за последние 30 дней. Сначала изучи показатели, мастеров и финансы, затем подключи контролёра. Дай приоритетный план на 14 дней.";
  }
  if (command === "/instagram") {
    return "Создай практичный Instagram-план Maestro на 7 дней для роста просмотров и записей. Изучи показатели бизнеса за 30 дней, привлеки маркетолога и Instagram-продюсера. Дай календарь публикаций, а для трёх лучших Reels — готовые производственные пакеты через prepare_instagram_content. Не утверждай, что Higgsfield уже запущен.";
  }
  if (command === "/reel") {
    const topic = text.trim().replace(/^\/reel(?:@\w+)?\s*/i, "").trim();
    return `Подготовь один полностью готовый Reels для Maestro${topic ? ` на тему: ${topic}` : " для привлечения новых клиентов"}. Сначала привлеки Instagram-продюсера, затем используй prepare_instagram_content. Укажи хук, покадровый сценарий, реплики, текст на экране, английский prompt и negative prompt для Higgsfield, обложку, подпись, CTA, сторис-поддержку и KPI. Это черновик: генерацию не считать запущенной без подтверждения владельца.`;
  }
  return text.trim();
}

function money(value: number): string {
  return Math.round(value || 0).toLocaleString("ru-RU");
}

function delta(value: number | null | undefined): string {
  if (value == null) return "нет базы сравнения";
  const sign = value > 0 ? "+" : "";
  return `${sign}${value.toFixed(1)}%`;
}

export function formatBusinessSummary(report: BusinessSummaryReport): string {
  const { current, changePercent, period } = report;
  return [
    `📊 Maestro: ${period.from} — ${period.to}`,
    "",
    `Выручка: ${money(current.revenue)} сум (${delta(changePercent.revenue)})`,
    `Клиенты: ${current.clients} (${delta(changePercent.clients)})`,
    `Новые: ${current.newClients} · постоянные: ${current.returningClients}`,
    `Средний чек на клиента: ${money(current.averagePerClient)} сум (${delta(changePercent.averagePerClient)})`,
    `Продаж: ${current.transactions}`,
    "",
    `Оплата: наличные ${money(current.cash)} · карта ${money(current.card)} · QR ${money(current.qr)}`,
    "",
    "Сравнение выполнено с предыдущим периодом такой же длины."
  ].join("\n");
}
